...
// 外部函数声明 - 同步mainLogic界面的SeekBar
//extern void set_main_voiceSeekBar(int progress);
//extern void set_main_brightnessSeekBar(int progress);
static bool Activity_is_load = false;
static bool sLongClickHandled = false;
static long sDownTime = 0;
static const long LONG_CLICK_THRESHOLD = 500;
...
void showSysInfo(ZKTextView* textViewPtr) {
	// LOGD("gannina navibar btincoming memory");
	struct sysinfo info;
	sysinfo(&info);
...
public:
    ...
    {
        sLongClickHandled = true;  // 标记长按已处理
        if(!can_long_click)
            return;
	...
	virtual void onProgressChanged(ZKSeekBar *pSeekBar, int progress) {
		if (pSeekBar->getID() == mPlayVolSeekBarPtr->getID()) {
			if (sys::setting::get_fm_switch()) {
				...
				// 同步到mainLogic界面
//				set_main_voiceSeekBar(progress);
			}
			...
			// 同步到mainLogic界面
//			set_main_brightnessSeekBar(progress);
		}
	}
...
static void _audio_output_mode_cb(audio_player_mode_e new_mode, audio_player_mode_e old_mode) {
	...
	case E_AUDIO_PLAYER_MODE_FM:
		...
		LOGD("audio::get_system_vol() = %d",(int)(audio::get_system_vol() * 100));
		uart::fm_switch(true);
